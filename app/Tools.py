...
class Tools(QtCore.QObject):
    ...
    searched_string_signal = QtCore.Signal(object)  # On créé un signal 
    signal_results_query_search = QtCore.Signal(object)  # On créé un signal 
    signal_textEdit_setText = QtCore.Signal(object)  # On créé un signal 
    ...
    def is_db_empty(self):
        with DatabaseGestionSqlite.DatabaseGestionSqlite() as db_connection:  # with allows you to use a context manager that will automatically call the disconnect function when you exit the scope
            if db_connection.error_db_connection is None:
                #TPO#self.window_instance.textEdit.setText("Vérification en cours que la base de données n'est pas vide...")
                ...
                if db_connection.error_db_execution is None:
                    ...
                    if not rows_vmware:
                        #TPO#self.window_instance.textEdit.setText("Base VMware vide.\nImporter un export par Fichier > Importer un export...")
                        return True
                else:
                ...
                if db_connection.error_db_execution is None:
                    ...
                    if not rows_opca:
                        #TPO#self.window_instance.textEdit.setText("Base OPCA vide.\nImporter un export par Fichier > Importer un export...")
                        return True
                else:
            ...
            if db_connection.error_db_connection is None:
                ...
                if self.categorie_to_search_in == 'Equipement':
                    ...
                    if self.is_db_empty():
                        ...
                        #self.signal_textEdit_setText.emit("Recherche en cours...")
                        #TPO#QtWidgets.QApplication.processEvents()  # Force a refresh of the UI
                        logging.debug(f"self.search_list: {self.search_list}")

                        if not self.search_list:  # If search is empty, search and display all results
                        ...
                        if self.runs:  # Si on a appuyé sur Annuler, on ferme le thread proprement mais il continue de tourner pour finir sa tâche donc on met un flag pour lui dire qu'on a appuyé sur Annuler
                            ...
                            for nbr, result_query_search in enumerate(results_query_search, 1):
                                #logging.debug(f"result_query_search: {result_query_search}")
                                ...
                                self.list_result_saut = "\n".join(list_result)
                            #logging.debug(f"self.list_result_saut: {self.list_result_saut}")
